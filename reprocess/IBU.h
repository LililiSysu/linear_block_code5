...
/*****************************************************************//**
 * \file   OSD.h
 * \brief  Iterative Basis Update class
 *
 * \author 26259
 * \date   January 2024
 *********************************************************************/
...
class IBUc {		// delete this, to be done
protected:
	...
	Matrix<int> r_abs_bar_B_temp_record_accumulated;
...
public:
	...
	void solve(const Matrix<GF2>& Gs_ini, const Matrix<my_double>& r) {
		...
		r_abs_bar_B_temp_record_accumulated = r_abs_Gs_record;
		r_abs_bar_B_temp_record_accumulated.permute(r_abs_bar_B_temp_record);
	...
	void IBU_core() {
		...
		r_abs_Gs_B_sList.resize(1, k);
		for (int i = 0; i < k; ++i) {
			r_abs_Gs_B_sList[i] = r_abs_bar_B_temp[k - 1 - i];		// less first in sList
		}
		r_abs_Gs_B_sList.build();

		//r_abs_Gs_B_sList.set_by(r_abs_Gs_B);						// sList structure

		//cout << "r_abs_Gs_B_sList" << r_abs_Gs_B_sList;

		// mu is the index corresponding to largest element in 'r_abs_Gs_P'		
		for (int mu = k; r_abs_Gs_B_sList.empty() == false && mu < n && \
			r_abs_Gs_B_sList.start_val() < r_abs_bar_B_temp[mu]; mu++) {

			GF2_auxiliary_storage::iteration_number++;

			// determine mu and lambda
			int mu_real = r_abs_bar_B_temp_record[mu];
			int lambda = r_abs_Gs_B_sList.get_start_ind();
			int lambda_real;

			bool is_remove_start = true;				// this is for data sturcture 'sList'
			if (lambda != -1) {
				lambda_real = r_abs_bar_B_temp_record[k - 1 - lambda];

				while (Gs(lambda_real, mu_real) == 0) {
					// find the 'lambda_real' such that 'Gs(lambda_real, mu_real) == 1'

					if (is_remove_start == true) {
						r_abs_Gs_B_sList.move_start();
						is_remove_start = false;
					}
					else {
						r_abs_Gs_B_sList.move_next();
					}

					lambda = r_abs_Gs_B_sList.get_next_ind();
					if (lambda != -1) {
						lambda_real = r_abs_bar_B_temp_record[k - 1 - lambda];
					}
					else {
			...
			if (lambda != -1 && r_abs_Gs_B_sList[lambda] < r_abs_bar_B_temp[mu]) {
				...
				for (int i = 0; i < k; ++i) {
					...
					if (Gs[i_row_start_ind + mu_real] == 1) {	// if (Gs(i,mu)==1)
						...
						for (int j = k; j < n; ++j) {		// add row-lamda of P to row-i of P, where P is parity submatrix of 'Gs'
				...
				// Update non-MRIP candidates, removing MRIP
				if (is_remove_start == true) {
					r_abs_Gs_B_sList.remove_start();
				}
				else {
					r_abs_Gs_B_sList.remove_next();
				}

				// Record the swapped element for merge sorting
			}

			// Update MRIP candidates, removing non-MRIP in r_Gs_P
		}
