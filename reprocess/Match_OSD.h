/*****************************************************************//**
 * \file   Match_OSD.h
 * \brief  New idea of matching TEP according to TEP (abandon)
 * 
 * \author 26259
 * \date   March 2024
 *********************************************************************/
...
protected:
	...
	Matrix<GF2> G_ext_delta;			// extended information set by delta position, incuring delta checks 'H_info_delta'
	Matrix<GF2> H_info_delta;			// delta checks for extended infomation set
...
public:
	...
	Match_OSD(const Matrix<GF2>& _G, const Matrix<GF2>& _H, int _d, int _order, int _delta \
		...
		k_prime = k + delta;
		k_shrink = k - delta;
		G_ext_delta = Matrix<GF2>(k_prime, k_prime, 'i').combine_right(Matrix<GF2>(k_prime, n - k_prime, '0'));
		H_info_delta = Matrix<GF2>(delta, k, '0').combine_right(Matrix<GF2>(delta, delta, 'i'));
	...
	void solve(const Matrix<my_double>& r) {
		...
		// set 'G_ext_delta' and 'H_info_delta', start from 'Gs' any way
		for (int i = 0; i < k; ++i) {
			for (int j = k_prime; j < n; ++j) {
				G_ext_delta(i, j) = Gs(i, j);
			}
		}
		for (int i = 0; i < delta; ++i) {
			for (int j = 0; j < k; ++j) {
				H_info_delta(i, j) = Gs(j, i + k);
			}
		}

		//cout << "G_ext_delta" << G_ext_delta;
		//cout << "H_info_delta" << H_info_delta;
		...
		for (int j = 0; j < k_prime; ++j) {
			...
			if (y_Gs[j] == 1) {		// consider to write it as 'yB_Gs[j] + 1 == 0' for counting one GF2 operation
				...
				for (int p = k_prime; p < n; ++p) {
					c0_Gs[p] += G_ext_delta[row_start_ind + p];		// add accumulately, this cause my second problem, mark down here
				}
			}
	...
	void re_encoding(const Matrix<int>& TEP_now) {
		...
		for (int j = 0; j < TEP_now.size(); ++j) {
			...
			for (int p = k_prime; p < n; ++p) {
				ce_Gs[p] += G_ext_delta[row_start_ind + p];		// add accumulately, this cause my second problem, mark down here
			}
		}
	...
	unsigned long long compute_check_bin(const Matrix<int>& TEP_input) {
		...
		// compute the inner product for each row in 'H_info_delta'
		for (int i = 0; i < delta; ++i) {
			int TEP_w = TEP_input.size();
			GF2 row_result = 0;
			for (int j = 0; j < TEP_w; ++j) {
				row_result += H_info_delta(i, TEP_input(j));
			}
